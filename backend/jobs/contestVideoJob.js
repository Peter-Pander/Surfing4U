// backend/jobs/contestVideoJob.js
...
async function pickWithAI(videos) {
  ...
  const chatRes = await openai.chat.completions.create({
    ...
    messages: [
      {
        role:    "system",
        content: "You are a surf expert. Pick the clip most likely to show an epic wave-riding highlight."
      },
      {
        role:    "user",
        content: `Here are several videos for ${contestName}:\n\n${prompt}\n\nReply with the NUMBER only.`
      }
    ],
    temperature: 0.7,
    max_tokens:  4,
...
async function runContestJob() {
  try {
    ...
    // write to root-level contestHighlight.json
    const outPath = path.join(__dirname, "..", "contestHighlight.json");
    fs.writeFileSync(outPath, JSON.stringify({ contestName, ...pick }, null, 2));
